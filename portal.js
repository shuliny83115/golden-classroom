(() => {
  ...
  function renderDashboard(){
    const {profile:p,people=[],lessons=[],students=[]}=dashboard;const admin=p.role==='admin';
  ...
  function renderLessons(){if(!dashboard)return;const person=$('personFilter').value;const from=$('fromDate').value,to=$('toDate').value;
    if(from&&to&&from>to){$('lessonList').innerHTML='<div class="empty"><strong>日期範圍有誤</strong><p>結束日期需晚於或等於起始日期。</p></div>';return;}
    const lessons=(dashboard.lessons||[]).filter(l=>(!from||dateKey(l.starts_at)>=from)&&(!to||dateKey(l.starts_at)<=to)&&(!person||[l.teacher_id,l.student_id].includes(person)));
  ...
  setInterval(()=>{if(dashboard&&!document.hidden&&activeView==='schedule'&&!document.querySelector('dialog[open]'))renderLessons();},1000);
  ...
  function renderPeople(){const {people,rooms}=dashboard;$('peopleRows').innerHTML=people.map(p=>`<tr><td>${esc(p.display_name)}<small>${esc(p.username||'既有測試帳號')}</small></td><td>${roles[p.role]}</td><td><span class="badge ${esc(p.status)}">${statuses[p.status]}</span></td><td>${esc(p.role==='teacher'?rooms.find(r=>r.id===p.fixed_room_id)?.room_name||'尚未指派':p.role==='student'?people.find(t=>t.id===p.assigned_teacher_id)?.display_name||'尚未指派':'—')}</td><td>${p.role!=='admin'?`<button class="subtle" data-edit="${esc(p.id)}" aria-label="編輯 ${esc(p.display_name)}">編輯</button>`:''}</td></tr>`).join('');}
  document.querySelectorAll('[data-view]').forEach(button=>button.onclick=()=>{activeView=button.dataset.view;document.querySelectorAll('[data-view]').forEach(b=>b.classList.toggle('active',b===button));['schedule','applications','people','audit','courses'].forEach(v=>$(v+'Panel').hidden=v!==activeView);message('portalMessage','');if(activeView==='courses')loadCourses().catch(e=>message('courseMessage',friendly(e)));if(activeView==='audit')loadAudit().catch(()=>{});if(activeView==='applications')loadDashboard().catch(()=>{});});
  ...
  $('peopleRows').onclick=event=>{const b=event.target.closest('[data-edit]');if(!b)return;editing=dashboard.people.find(p=>p.id===b.dataset.edit);const p=editing;
